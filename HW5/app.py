...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:4Rocks32@localhost/rachjrHW5"
## Provided:
...
def update(item):
    ...
    if form.validate_on_submit():
        ...
        ti = TodoItem.query.filter_by(description = item).first()
        ti.rating = new_rating
        db.session.commit()
...
def delete(lst):
    form = DeleteButtonForm()
    if form.validate_on_submit():
        td = TodoList.query.filter_by(title = lst).first()
